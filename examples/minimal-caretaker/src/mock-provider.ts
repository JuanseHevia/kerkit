...
/**
 * A scripted "model" so the demo runs with no API key: on the first round it
 * calls a kerkit tool chosen from the user's message; on the second it
...
export class MockProvider implements ProviderAdapter {
  async generate(opts: GenerateOptions): Promise<ProviderTurn> {
    ...
    const tool = message.includes('trámite') || message.includes('autoriza')
      ? 'read_authorizations'
